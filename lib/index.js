...
const passwordRegEx = /^[\x21-\x7E]{6,}$/

/**
 * Parse a valid RFC3339 date string into a `Date` object.
 *
 * @private
 * @since 0.3.0
...
/**
 * Verify if someone's age falls within a minimum and maximum age values.
 *
 * @public
 * @since 0.3.0
 * @param {String} dob - A valid RFC3339 date string.
 * @param {Number} [minAge=0] - The minimum accepted age value.
 * @param {Number} [maxAge] - The maximum accepted age value.
 * @return {Boolean|Number} - Returns `false` if the minimum or maximum age are not valid, or the calculated age if valid.
 */
export function age (dob, minAge = 0, maxAge) {
  const now = new Date()
...
/**
 * Verify if a password has only valid characters and has a minimum length of 6 characters.
 * The valid characters are the ASCII printable characters, excluding the space character.
 *
 * @public
 * @since 0.4.0
 * @param {!String} value - The value to be validated.
 ...
 *
 * @example
 * var isPasswordValid = defy.password('WowqGY0Gi?oaPnT,MwiJ[kBQ4ID29Hb!')
 */
...
export function required (value) {
  ...
  if (typeof value === 'string') {
    inputValue = cleanValue(inputValue)
  }
